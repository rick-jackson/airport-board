...
const ToggleDate = ({setSearchDate, searchDate}) =>{
  ...
  useEffect(()=>{
    ...
    }else if(date === moment(new Date()).subtract(1, 'day').format('DD-MM-Y')){
      setLinePs({left: '0%'})
...
return(
  <div className="date-container">
    ...
    <input className="date-container__input" type={'date'}  onChange={handleChange}></input>
    </div>

  <div className="day-wrapper">
    <div className="day-container">
    <div className="day-wrapper__container"  onClick={()=>setNewDate((moment(new Date()).subtract(1, 'day').format('DD-MM-Y')))}>
      <div className="day-wrapper__date">{moment(new Date()).subtract(1, 'day').format('DD/MM')}</div>
      <div className="day-wrapper__day-name">YESTERDAY</div>
    </div>
    <div className="day-wrapper__container" onClick={()=>setNewDate((moment(new Date()).format('DD-MM-Y')))}>
      <div className="day-wrapper__date">{moment(new Date()).format('DD/MM')}</div>
      <div className="day-wrapper__day-name">TODAY</div>
    </div>
    <div className="day-wrapper__container"  onClick={()=>setNewDate((moment(new Date()).add(1, 'day').format('DD-MM-Y')))}>
      <div className="day-wrapper__date">{moment(new Date()).add(1, 'day').format('DD/MM')}</div>
